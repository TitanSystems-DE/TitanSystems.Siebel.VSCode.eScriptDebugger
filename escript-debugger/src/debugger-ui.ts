...
function configureWebview(webview: vscode.Webview, context: vscode.ExtensionContext, store: ProfileStore, actions: UiActions): void {
  ...
  const render = () => { webview.html = html(webview, context, store, program); };
  render();
  webview.onDidReceiveMessage(async message => {
    switch (message?.command) {
      ...
      case 'useActiveFile': program = currentProgram(); render(); break;
...
function html(webview: vscode.Webview, context: vscode.ExtensionContext, store: ProfileStore, program: string): string {
...
</main><script nonce="${nonce}">const vscode=acquireVsCodeApi(),program=${JSON.stringify(program)};let mode='standalone';const byId=id=>document.getElementById(id),setMode=value=>{mode=value;byId('standaloneMode').classList.toggle('selected',value==='standalone');byId('serviceMode').classList.toggle('selected',value==='service');byId('serviceConfig').classList.toggle('hidden',value!=='service');byId('modeLabel').textContent='Mode: '+(value==='service'?'Service':'Standalone')};byId('standaloneMode').onclick=()=>setMode('standalone');byId('serviceMode').onclick=()=>setMode('service');document.querySelectorAll('input[name="entry"]').forEach(r=>r.onchange=()=>{byId('invokeConfig').classList.toggle('hidden',r.value!=='invokeMethod'||!r.checked);byId('directConfig').classList.toggle('hidden',r.value!=='direct'||!r.checked)});const addInput=(name='',value='')=>{const row=document.createElement('div');row.className='input-row';const n=document.createElement('input');n.placeholder='Name';n.value=name;const v=document.createElement('input');v.placeholder='Value';v.value=value;const remove=document.createElement('button');remove.className='secondary remove';remove.textContent='×';remove.onclick=()=>row.remove();row.append(n,v,remove);byId('inputs').append(row)};byId('addInput').onclick=()=>addInput();const launch=noDebug=>{const entryPoint=document.querySelector('input[name="entry"]:checked').value,inputs=[...document.querySelectorAll('.input-row')].map(row=>({name:row.children[0].value,value:row.children[1].value}));vscode.postMessage({command:'launch',mode,program,connection:byId('connection').value,noDebug,entryPoint,directFile:byId('directFile').value,methodName:byId('methodName').value,inputs})};byId('debug').onclick=()=>launch(false);byId('run').onclick=()=>launch(true);byId('choose').onclick=()=>vscode.postMessage({command:'chooseFile'});byId('active').onclick=()=>vscode.postMessage({command:'useActiveFile'});byId('connections').onclick=()=>vscode.postMessage({command:'manageConnections'});window.addEventListener('message',event=>{if(event.data.command!=='serviceOutputs')return;const entries=Object.entries(event.data.outputs||{}),tbody=byId('outputs').querySelector('tbody');tbody.textContent='';for(const [name,value]of entries){const row=tbody.insertRow(),a=row.insertCell(),b=row.insertCell();a.textContent=name;b.textContent=value}byId('outputs').classList.toggle('hidden',!entries.length);byId('outputEmpty').classList.toggle('hidden',entries.length>0);if(!entries.length)byId('outputEmpty').textContent='InvokeMethod completed without output properties.'});</script></body></html>`;
}
